...
const POLL_WEIGHTS = [
  ...
  { id: "22차", org: "에이스리서치/부산일보", method: "ARS", n: 502, rr: 10.9, period: "5/23~24", mW: 1, sW: 1, rrW: 1.1, tW: 1.8, score: 79, tier: "high" },
];
...
const REFERENCE_DATA = [
  {
    ...
    items: [
      "9~15차 (5/16~24, 7건): 전재수 42~50% / 박형준 29~43% — 전재수 우위 지속",
      "15차(뉴스핌/리얼미터 5/23~24): 전재수 44.8% / 박형준 42.8% — 2.0%p 초박빙, 최근 격차 축소",
      "3권역(강서·북·사상, 북구 포함): 전재수 43.8% / 박형준 42.2% — 북구 포함 권역 초박빙",
      "40~50대 전재수 압도, 60대·70대+ 박형준 우세, 18~29세 박형준 49.9% — 청년층 보수 결집",
      "적극 투표층 전재수 50.3% / 박형준 44.3% — 결집층 민주 우위, 무당층 동률(29%대)",
    ],
  },
  {
    title: "일별 부산 동향",
        ...
        {activeSection === "weights" && (
          <div>
            <div className="mb-8">
              ...
              <p className="text-sm text-[var(--text)]">1~22차 여론조사를 신뢰도에 비례해 가중 평균으로 통합합니다. 5/15 이후 최근 조사에 x1.8 최신성 가중치 적용.</p>
            </div>
              ...
              <div className="bg-[var(--code-bg)] border border-[var(--border)] rounded-xl overflow-hidden">
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {
    [
      ["18~29세", 28.7, 28.0, 22.7, 20.5],
      ["30대", 32.8, 27.7, 24.3, 15.2],
      ["40대", 52.4, 24.5, 14.3, 8.7],
      ["50대", 44.5, 29.3, 20.9, 5.3],
      ["60대", 36.4, 35.2, 22.6, 5.9],
      ["70대+", 27.6, 38.4, 25.5, 8.5],
    ].map(([band, h, k, p, m]) => (
          ...
          <div>
            ...
            <Section title="personas.db 구성">
              <div className="bg-[var(--code-bg)] border border-[var(--border)] rounded-xl divide-y divide-[var(--border)]">
                {[
                  ["총 유권자 수",    "122,440명"],
                  ["현재 poll_version", "22차 (에이스리서치/부산일보 기준)"],
                  ["마지막 업데이트", "2026-05-27"],
                  ["지지후보 배분",   "하정우 36.9% / 한동훈 31.4% / 박민식 22.0% / 미정 9.8%"],
