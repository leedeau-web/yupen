// 부산 북구갑 유권자 목업 데이터 생성기 v3
// 출처:
//   선거인수·동별비율 - 22대 총선 중앙선거관리위원회 공개 데이터 (122,440명)
//   연령별 분포       - 부산 북구 주민등록 인구통계 (18세+ 기준)
//   지지율            - 3개 여론조사 평균: KOPRA 4/19~20, 미디어토마토 4/24~25, 여론조사꽃 4/26~27
...
// ── ① 연령×후보 그룹 정의 ────────────────────────────────────────────────────
//
// 연령 분포 (부산 북구 18세+ 인구통계, 정규화 후 1000명):
//   18-29세: 130명 (12.0%)
//   30-39세: 130명 (11.8%)
//   40-49세: 135명 (12.3%)
//   50-59세: 185명 (16.9%)
//   60-69세: 225명 (21.3%)
//   70세이상: 195명 (18.2%)   합계 = 1000
//
// 후보 분포 (3개 조사 평균):
//   하정우 368명(36.8%)  한동훈 258명(25.8%)  박민식 238명(23.8%)  미정 136명(13.6%)
//
// 연령내 후보배분 (미디어토마토 2차 + 여론조사꽃 3차 조사 평균):
//   18-29: 하41 한39 박32 미18 = 130
//   30-39: 하41 한38 박33 미18 = 130
//   40-49: 하66 한23 박28 미18 = 135   (40대 하정우 강세 반영)
//   50-59: 하71 한41 박48 미25 = 185
//   60-69: 하91 한60 박43 미31 = 225
//   70이상: 하58 한57 박54 미26 = 195  (70대+ 박·한 혼전)
//
const ageGroups = [
  { ageMin: 18, ageMax: 29, candidate: '하정우', count: 41 },
  { ageMin: 18, ageMax: 29, candidate: '한동훈', count: 39 },
  { ageMin: 18, ageMax: 29, candidate: '박민식', count: 32 },
  { ageMin: 18, ageMax: 29, candidate: '미정',   count: 18 },

  { ageMin: 30, ageMax: 39, candidate: '하정우', count: 41 },
  { ageMin: 30, ageMax: 39, candidate: '한동훈', count: 38 },
  { ageMin: 30, ageMax: 39, candidate: '박민식', count: 33 },
  { ageMin: 30, ageMax: 39, candidate: '미정',   count: 18 },

  { ageMin: 40, ageMax: 49, candidate: '하정우', count: 66 },
  { ageMin: 40, ageMax: 49, candidate: '한동훈', count: 23 },
  { ageMin: 40, ageMax: 49, candidate: '박민식', count: 28 },
  { ageMin: 40, ageMax: 49, candidate: '미정',   count: 18 },

  { ageMin: 50, ageMax: 59, candidate: '하정우', count: 71 },
  { ageMin: 50, ageMax: 59, candidate: '한동훈', count: 41 },
  { ageMin: 50, ageMax: 59, candidate: '박민식', count: 48 },
  { ageMin: 50, ageMax: 59, candidate: '미정',   count: 25 },

  { ageMin: 60, ageMax: 69, candidate: '하정우', count: 91 },
  { ageMin: 60, ageMax: 69, candidate: '한동훈', count: 60 },
  { ageMin: 60, ageMax: 69, candidate: '박민식', count: 43 },
  { ageMin: 60, ageMax: 69, candidate: '미정',   count: 31 },

  { ageMin: 70, ageMax: 88, candidate: '하정우', count: 58 },
  { ageMin: 70, ageMax: 88, candidate: '한동훈', count: 57 },
  { ageMin: 70, ageMax: 88, candidate: '박민식', count: 54 },
  { ageMin: 70, ageMax: 88, candidate: '미정',   count: 26 },
];

// 합계 검증
const totalAgeGroup = ageGroups.reduce((s, g) => s + g.count, 0);
if (totalAgeGroup !== 1000) throw new Error(`연령×후보 합계 오류: ${totalAgeGroup} (expected 1000)`);

// ── ② 동별 정확 슬롯 ─────────────────────────────────────────────────────────
// 22대 총선 선관위 공개 데이터 비율 기준 (총 122,440명)
// 최대잉여법(Largest Remainder) 적용 → 정확히 1000명
//   구포1동 116.1→116  구포2동 175.8→176*  구포3동 138.3→138
//   덕천1동  90.1→ 90  덕천2동  86.8→ 87*  덕천3동  76.5→ 77*
//   만덕2동 186.3→186  만덕3동 130.0→130
const dongSlots = buildSlots({
  구포1동: 116, 구포2동: 176, 구포3동: 138,
  덕천1동:  90, 덕천2동:  87, 덕천3동:  77,
  만덕2동: 186, 만덕3동: 130,
}); // 합계: 116+176+138+90+87+77+186+130 = 1000

// ── ③ 성별 정확 슬롯 ─────────────────────────────────────────────────────────
// 부산 북구 선거인 성별 비율: 남 49.15%, 여 50.85%
// 남 1000×0.4915 = 491.5 → 492명, 여 508명
const genderSlots = buildSlots({ 남: 492, 여: 508 });
...
console.log('  출처: 선관위 22대 총선 + 3개 여론조사 평균 (KOPRA·미디어토마토·여론조사꽃)\n');

console.log('── 지지후보 (목표 vs 실제) ──────────────────────────────');
[['하정우',368,'36.8'],['한동훈',258,'25.8'],['박민식',238,'23.8'],['미정',136,'13.6']].forEach(([c,t,tp]) => {
  const a = cnt('지지후보', c);
  console.log(`  ${c}: ${a}명 (${pct(a)}) ← 목표 ${t}명 (${tp}%)`);
});

console.log('\n── 연령대 (목표 vs 실제) ───────────────────────────────');
[[18,29,'18-29세',130,12.0],[30,39,'30-39세',130,11.8],[40,49,'40-49세',135,12.3],
 [50,59,'50-59세',185,16.9],[60,69,'60-69세',225,21.3],[70,99,'70세이상',195,18.2]]
...
console.log('\n── 성별 (목표 vs 실제) ─────────────────────────────────');
[['남',492,'49.2'],['여',508,'50.8']].forEach(([g,t,tp]) => {
  const a = cnt('성별', g);
  console.log(`  ${g}: ${a}명 (${pct(a)}) ← 목표 ${t}명 (${tp}%)`);
});

console.log('\n── 거주동 (목표 vs 실제) ───────────────────────────────');
[['구포1동',116,11.6],['구포2동',176,17.6],['구포3동',138,13.8],
 ['덕천1동', 90, 9.0],['덕천2동', 87, 8.7],['덕천3동', 77, 7.7],
 ['만덕2동',186,18.6],['만덕3동',130,13.0]]
.forEach(([d,t,tp]) => {
  const a = cnt('거주동', d);
